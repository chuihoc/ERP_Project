...
class EditableTable extends React.Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      {
        title:
          ans_language.ans_config_default_title || "ans_config_default_title",
      ...
      {
        title:
          ans_language.ans_config_custom_title || "ans_config_custom_title",
        dataIndex: "config",
        //width: '40%',
        editable: true,
        required: true
              ...
              {editable ? (
                <span>
                  <EditableContext.Consumer>
                    {form => (
                      ...
                      >
                        <Icon type="save" />
                        {ans_language.ans_save || "ans_save"}
                      </a>
                    )}
                  </EditableContext.Consumer>
                  <Popconfirm
                    title={
                      ans_language.ans_confirm_cancel || "ans_confirm_cancel"
                    }
                  ...
                  >
                    ...
                    <a href="javascript:;">
                      <Icon type="close" />
                      {ans_language.ans_cancel || "ans_cancel"}
                    </a>
                  </Popconfirm>
                </span>
              ) : (
                <React.Fragment>
                  <a href="javascript:;" onClick={() => this.edit(record.key)}>
                    <Icon type="edit" />
                    {ans_language.ans_edit || "ans_edit"}
                  </a>
                  {/* {" | "}
                  <Popconfirm
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        fetch(window.ISD_BASE_URL + "opts/updateOpts", {
          method: "POST",
  ...
  delete = record => {
    if (record.id) {
      fetch(window.ISD_BASE_URL + "opts/deleteOpts/" + record.id, {
        headers: getTokenHeader()
      })
  ...
  fetchData() {
    fetch(window.ISD_BASE_URL + "opts/fetchOpts", {
      headers: getTokenHeader()
    })
      .then(response => {
        return response.json();
    ...
    if (searchText) {
      data = data
        .map(record => {
          ...
          let fullText = `${record.ma_opt}${record.defaultconfig}${
            record.config
          }`;
          ...
          <Row>
            <Col span={12}>
              <h2 className="head-title">
                {ans_language.ans_options_title || "ans_options_title"}
              </h2>
            </Col>
            <Col span={12}>
              <div className="action-btns">
                <Button
                  ...
                  icon="plus"
                >
                  {ans_language.ans_add_new || "ans_add_new"}
                </Button>
              </div>
            </Col>
          </Row>
