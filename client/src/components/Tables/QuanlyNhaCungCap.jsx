...
class QuanlyNhaCungCap extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        fetch(window.ISD_BASE_URL + "nha_cung_cap/updateNpp", {
          method: "POST",
  ...
  delete = record => {
    if (record.id) {
      fetch(window.ISD_BASE_URL + "nha_cung_cap/deleteNpp/" + record.id, {
        headers: getTokenHeader()
      })
  ...
  fetchData() {
    fetch(window.ISD_BASE_URL + "nha_cung_cap/fetchNpp", {
      headers: getTokenHeader()
    })
      .then(response => {
        return response.json();
  ...
  render() {
    ...
    if (searchText) {
      data = data
        .map(record => {
          let fullText = `${record.ma_npp}${record.name}${record.address}${
            record.phone
          }${record.ranking}`;
