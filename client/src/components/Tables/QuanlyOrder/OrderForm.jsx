...
class OrderForm extends React.Component {
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    this.props.form.validateFieldsAndScroll((err, values) => {   console.log(values);
      if (!err) {
        //Get USER ID from mainstate
        let {systemOrder} = this.props.mainState;
        values.id = systemOrder.id ? systemOrder.id : '';
        //values.date_delive.format('YYYY-MM-DD');
        fetch(window.ISD_BASE_URL + 'order/updateDh', {
          method: 'POST',
  ...
  fetchProductListbyCate(cateId){
    fetch(window.ISD_BASE_URL + `sx/fetchProductByCate/${cateId}`, {
      headers: getTokenHeader()
    })
        ...
        <Row className="ans_row_margin_10">
          <Col span={12}>
            <FormItem
              ...
              label={(
                <span>
                  {ans_language.ans_product_name || 'ans_product_name'} 
                </span>
              )}
            > 
            {getFieldDecorator('product_id', {
              initialValue: systemOrder.product_id,
              rules: [{ required: true, message: ans_language.ans_choose_product || 'ans_choose_product' }],
            })(
              <Select 
        ...
        <Row>
          <Col span="12">
              <FormItem
                ...
                >
                {getFieldDecorator('date_delive', {
                  initialValue: date_delive,
                  rules: [{ required: true, message: ans_language.ans_pls_enter_title || 'ans_pls_enter_title'}],
                })(
          ...
          <Col span="12">
            <FormItem
              ...
              label={(
                <span>
                  {ans_language.ans_order_price || 'ans_order_price'} &nbsp;
                  <Tooltip title={ans_language.ans_order_price_note || 'ans_order_price_note' }>
                    <Icon type="question-circle-o" />
                  </Tooltip>
                </span>
              )}
            >
              {getFieldDecorator('price', {
                initialValue: systemOrder.price,
                rules: [{ required: true, message: ans_language.ans_pls_enter_title || 'ans_pls_enter_title' , whitespace: true }],
              })(
                <Input className="ans_input_80" />
              )}
        ...
        <Row>
          <Col span={24}>
            <FormItem
              ...
              label= {
                <span>
                  {ans_language.ans_order_note || 'ans_order_note'} 
                </span>
              }
              >
