...
class GanttController extends BaseController {
  private $tableName = 'gantt_tasks';
	...
	protected function getTasksByMaOrder($maOrder) {
		$tasks = [
			'data' => [],
			'links' => []
		];
	...
	protected function getTasksByUser($user) {
		$tasks = [
			'data' => []
		];
		if($user) {
  ...
  public function fetchTasksByMaSx($request, $response, $args){
	...
	public function fetchTasksByMaOrder($request, $response, $args){
		$this->checkUser();
    //$this->logger->addInfo('Request Npp path');
		$rsData = array(
			'status' => self::ERROR_STATUS,
    ...
    $maOrder = isset(	$args['ma_order']) ? $args['ma_order'] : '';
    if($maOrder == "") {
      $rsData['message'] = 'Không tìm thấy mã hoá đơn!';
      echo json_encode($rsData);
      die;
		};
		
		$collection = $this->getTasksByMaOrder($maOrder);
		if(!empty($collection['data'])) {
			$rsData['status'] = self::SUCCESS_STATUS;
      $rsData['message'] = 'Dữ liệu đã được load!';
			$rsData['data'] = $collection;
	...
	public function fetchMyTasks($request, $response, $args){
    ...
    $userId = isset($this->jwt->id) ? $this->jwt->id : '';
    if($userId == "") {
      $rsData['message'] = 'Không tìm thấy mã nhân viên!';
      echo json_encode($rsData);
      die;
		};
		
		$collection = $this->getTasksByUser($userId);
		if(!empty($collection['data'])) {
			$rsData['status'] = self::SUCCESS_STATUS;
      $rsData['message'] = 'Dữ liệu đã được load!';
			$rsData['data'] = $collection['data'];
		}
		echo json_encode($rsData);
	}
	/**
  ...
  public function update($request, $response){
    ...
    $parent = $request->getParam('parent');
		...
		$maOrder = $request->getParam('ma_order');
    ...
    $itemData = [
      ...
      $itemData['create_by'] = $userId;
		...
		$rsData['data'] = $this->getTaskByType($quyTrinhId, $maSx, $maRnd, $maOrder);
		echo json_encode($rsData);
	}
	/**
	 * Type: Quy trinh ID, Ma Sx or Ma Rnd or Ma Order
	 */
	protected function getTaskByType($quyTrinhId = '', $maSx = '', $maRnd = '', $maOrder = '') {
		if($quyTrinhId) {
			$collection = $this->getTasksByQuytrinhId($quyTrinhId);
		} elseif($maSx) {
			$collection = $this->getTasksByMaSx($maSx);
	...
	public function updateTaskOrder($request, $response){
		...
		if($targetId) {
			...
			if($maOrder) {
				$extraWhere .= "AND ma_order = '$maOrder'";
			}
