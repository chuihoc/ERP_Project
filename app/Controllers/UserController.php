...
class UserController extends BaseController {
  ...
  public function isSuperAdmin($userId = "") {
    ...
    if($userId != "") {
      $user = $this->db->select('ans_users', ['is_super'], ['id' => $userId]);
  ...
  public function getAllActiveUsers($userId = "") {
    // Columns to select.
		$columns = [
      'ans_users.id',
      'ans_users.id(key)',//Unique key for React loop
      'ans_users.name',
      'ans_users.username',
      //'hash',
      'ans_users.email',
      'ans_users.status',
      'ans_users.roles',
      'ans_users.group_user',
      'ans_users.ma_ns',
      'ans_users.to_hanh_chinh',
      'ans_users.phone',
      'ans_users.description',
      'ans_users.create_on',
      'ans_phongban.name(ten_phong_ban)',
      'ans_phongban.roles',
    ];
    $where = [
      "ORDER" => ["create_on" => "DESC"],
      "ans_users.status" => [0, 1]//2 is deleted status
    ];
    //Load user cu the theo ID 
    if($userId) {
      $where["ans_users.id"] = $userId;
    }
    $collection = $this->db->select($this->tableName,[
      "[>]ans_phongban" => ["group_user" => "ma_pb"],
    ], $columns, $where);
    if(!empty($collection)) {
      return $collection;
    }
  ...
  public function updatePermission($request, $response){
    ...
    $include = $request->getParam('include');
      ...
      $permissionExists = $this->db->select('ans_user_permission', ['user_id', 'router_name'], 
      ['user_id' => $userId, 'router_name' => $routerName]);
      if(!empty($permissionExists)) {
        //Update permission
        $result = $this->db->update('ans_user_permission', ['allow' => $allow, 'include' => $include], ['user_id' => $userId, 'router_name' => $routerName]);
      } else {
        //Add permission 
        $result = $this->db->insert('ans_user_permission', 
        [
          'user_id' => $userId, 
  ...
  public function getUserPermission($userId) {
    if($userId) {
      $collection = $this->db->select('ans_user_permission', ['router_name','include'], ['user_id' => $userId, 'allow' => '1']);
      return $collection;
    }
    return [];
  }
  ...
  public function getUserGroupRolesByUserId($userId) {
    ...
    $collection = $this->db->select($this->tableName, [
      "[>]ans_phongban" => ["group_user" => "ma_pb"],
    ], ["ans_users.id", "ans_phongban.roles"], ["ans_users.id" => $userId]);
