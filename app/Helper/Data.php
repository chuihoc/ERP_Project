...
class Data {
  ...
  public function findDistrict($sourceText, $districtList) {
    if($sourceText != "") {
      ...
      foreach($districtList as $district) {
        ...
        $districtTemp = str_replace('TP', '', $districtTemp);
        ...
        if(is_numeric($districtTemp)) {
          ...
          if(strpos($sourceText, $districtTemp) !== false) {
            //Reset mang so sanh các quận thuộc HCM
            $foundArr = [];
            $foundArr[] = $district;
          } else {
            $districtTemp = str_replace('quận', 'q', $districtTemp);
            if(strpos($sourceText, $districtTemp) !== false) {
              //Reset mang so sanh các quận thuộc HCM
              $foundArr = [];
              $foundArr[] = $district;
  ...
  public function getAllProductForExport($verify = true, $where = "") {
     ...
     * **/
    $sql = "SELECT `san_pham_theo_phieu`.`id`,`san_pham_theo_phieu`.`id` AS `key`,`san_pham_theo_phieu`.`ma_phieu`,`san_pham_theo_phieu`.`product_id`,(`san_pham_theo_phieu`.`sl_thucnhap` - IFNULL(`san_pham_theo_phieu_xuat`.`sl_thucnhap`,0)) AS 'sl_thucnhap',(`san_pham_theo_phieu`.`sl_thucnhap` - IFNULL(`san_pham_theo_phieu_xuat`.`sl_thucnhap`,0)) AS 'sl_ton',`san_pham_theo_phieu`.`qc_check`,`san_pham_theo_phieu`.`qa_check`,`san_pham_theo_phieu`.`vi_tri_kho`,`san_pham_theo_phieu`.`create_on`,`san_pham_theo_phieu`.`ngay_san_xuat`,`san_pham_theo_phieu`.`ngay_het_han`, lotus_vattu.name,`san_pham_theo_phieu`.`qc_note`,`san_pham_theo_phieu`.`qa_note`,`san_pham_theo_phieu`.`qc_file`,`san_pham_theo_phieu`.`qa_file`,`lotus_kho`.`ma_kho`, `lotus_kho`.`name` AS ten_kho, `lotus_vattu`.`category_id`
    FROM `san_pham_theo_phieu` 
    LEFT JOIN `phieu_nhap_xuat_kho` 
      ON `san_pham_theo_phieu`.`ma_phieu` = `phieu_nhap_xuat_kho`.`ma_phieu` 
    LEFT JOIN `lotus_kho` 
      ON `phieu_nhap_xuat_kho`.`ma_kho` = `lotus_kho`.`ma_kho`
    LEFT JOIN lotus_vattu
		  ON san_pham_theo_phieu.product_id = lotus_vattu.product_id 
